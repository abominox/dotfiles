...
alias yt2mp3="yt-dlp --ignore-errors --extract-audio --audio-format mp3 --add-metadata --embed-thumbnail -o '%(title)s.%(ext)s'"
...
# The following aliases from BASH have been converted to functions (see functions/ directory):
# - sherlock: Search command history
# - marlowe: Search Fish aliases and functions
# - fuck: Run last command with sudo
# - activate: Activate Python virtualenv (env/ or .env/)
# - ntest: Test nginx conf and restart service if successful

# The following BASH functions have been converted to Fish functions (see functions/ directory):
# - scrub: Delete lines with keyword from Fish history
# - x86: Run commands via Rosetta on M1 Macs
# - cheat: Search cheat.sh from terminal
# - trantest: Test transfer speed to directory/device
# - orph: Cleanup orphaned dependencies (Pacman)
