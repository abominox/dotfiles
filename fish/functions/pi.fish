...
#    (plus a symlink to libnode.141.dylib, since @rpath resolves relative to
#    the hard link's location).
# 2. The hard link is self-healing: if node is upgraded (different inode),
#    the function re-creates it on next launch.
...
# - ~/.local/share/pi/libnode.*.dylib  Symlink for @rpath (created automatically)
...
function pi --wraps=pi --description "Launch Pi coding agent (via node hardlink for tmux compat)"
    ...
    # Create/refresh the hard link and lib symlink on node upgrades
    if not test -f "$pi_node"
        or test (stat -f '%i' "$pi_node") != (stat -f '%i' "$node_bin")
        mkdir -p (dirname "$pi_node")
        ln -f "$node_bin" "$pi_node"
        # libnode.141.dylib is found via @rpath; the hard link changes the
    ...
    # Symlink chain: /opt/homebrew/bin/pi -> Cellar/bin/pi -> libexec/bin/pi -> dist/cli.js
    set -l pi_cli (realpath /opt/homebrew/bin/pi)
